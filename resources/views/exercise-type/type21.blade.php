...
@section('content')
    ...
    <div class="section-exercise-type section-exercise-type21 position-relative">
        ...
        @include('exercise-type.header_type', ['data' => $list_exercise_type_7, 'status_url'])
        @php
        ...
        <div class="board-question-content custom-media-query position-relative">
                <div class="question-boxes" onDragOver="dragOver( event )">
                    ...
                    @php
                        ...
                        $arrBGOrigin[] = $arrBackgrounds1[$key];
                    @endphp
                    <div draggable="true" onDragStart="dragStart(event)" data-bg-color="{{$value}}" class="sixth question-card {{$dataArr1['RESULT_CHILD'] ? 'disabled-active': ''}}" style="background-color: {{$value}}">
                        <div class="card-content">{{$arrayX[5]}}</div>
                    </div>
                    ...
                    <div class="sixth answer-card {{$dataArr1['RESULT_CHILD'] ? 'border-none': ''}}" onDragEnter="dragEnter(event)" onDragOver="dragOver( event )" onDrop="dragDrop(event)" style='{{$dataArr1['RESULT_CHILD'] ? 'background-color: '.$arrBGOrigin[5].';' : ''}}'>
                        @if($dataArr1['RESULT_CHILD'])
                            <div class="card-content">{{$arrayY[5]}}</div>
                        @else
                            <div class="answer-content">6</div>
                        @endif
                    </div>
                </div>
